...
function flattenDeep<T>(items: T[], iteratee: (arg: T) => T[] | undefined) {
    const flatten: T[] = [];
...
const props = withDefaults(
    defineProps<{
        toc: Toc; hideIndicator?: boolean; maxDepth?: number }>(), {maxDepth: 3}
);
...
const visibilityMap = ref<{
    [id: string]: { link: TocLink; visible: boolean; tocLink: HTMLElement; index: number }
}>({});
...
onMounted(() => {
    ...
    flattenLinks.forEach((l, i) => {
        ...
        if (!el) return;
        visibilityMap.value[l.id] = {
            link: l,
            visible: false,
            tocLink: navEl.value?.querySelector(`#link-${l.id}`)!,
            index: i
        };
        observer?.observe(el);
    });
